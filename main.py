...
from src.multisource_dataset import get_multisource_data_loaders
...
DEFAULT_BALANCED_CHECKPOINTS = {
    "DDPM": Path("checkpoints/efficientnet_balanced_holdout_ddpm_best.pt"),
    "ADM": Path("checkpoints/efficientnet_balanced_holdout_adm_best.pt"),
}
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--holdout",
        type=str.upper,
        choices=SUPPORTED_HOLDOUTS,
        default=None,
        help="Fake generator excluded from train-source-balanced training.",
    )
    parser.add_argument(
        "--samples-per-epoch",
        type=int,
    ...
    parser.add_argument(
        ...
        help="Deterministic source-balanced sampler seed.",
    )
...
def resolve_checkpoint_path(
    ...
    if command == "train-source-balanced":
        if holdout is None:
            raise ValueError("train-source-balanced requires --holdout ADM or DDPM.")
        return DEFAULT_BALANCED_CHECKPOINTS[normalize_holdout(holdout)]
...
def main() -> None:
    ...
    if args.command == "train-source-balanced" and args.holdout is None:
        parser.error("train-source-balanced requires --holdout ADM or DDPM")
    ...
    if args.command == "train-source-balanced":
        ...
        train_staged_model(
            ...
            stage2_epochs=args.stage2_epochs,
            checkpoint_path=checkpoint_path,
            heldout_generator=args.holdout,
        )
        return
